...
class DbManage {
    ...
    function backup($tablename = '', $dir='./', $size=20480) {
        ...
        // 创建目录
        if (! is_dir ( $dir )) {
            @mkdir ( $dir, 0755, true ) or die ( '创建文件夹失败' );
        }
        $size = $size ? $size : 1024*20;
        $sql = '';
        // 只备份某个表
        if (! empty ( $tablename )) {
            if(@mysqli_num_rows(mysqli_query($GLOBALS['conn'],"SHOW TABLES LIKE '".$tablename."'")) == 1) {
             } else {
            ...
            // 插入数据
            $data = mysqli_query ( $GLOBALS['conn'] , "select * from " . $tablename );
            // 文件名前面部分
            ...
            $this->_showMsg('正在备份');
            // 备份全部表
            if ($tables = mysqli_query ( $GLOBALS['conn'] , "show table status from " . $this->database )) {
            ...
            // 查出所有表
            $tables = mysqli_query ( $GLOBALS['conn'] , 'SHOW TABLES' );
            // 第几分卷
            $p = 1;
            // 循环所有表
    ...
    private function _retrieve() {
        ...
        $value .= '-- MySQL版本: ' . mysqli_get_server_info ($GLOBALS['conn']) . $this->ds;
    ...
    private function _insert_table_structure($table) {
        ...
        // 获取详细表信息
        $res = mysqli_query ( $GLOBALS['conn'] , 'SHOW CREATE TABLE `' . $table . '`' );
    ...
    private function _insert_record($table, $num_fields, $record) {
        ...
        for($i = 0; $i < $num_fields; $i ++) {
            $insert .= ($comma . "'" . mysqli_real_escape_string ( $GLOBALS['conn'] , $record [$i] ) . "'");
            $comma = ",";
        }
        $insert .= ");" . $this->ds;
        return $insert;
    ...
    private function _insert_into($sql){
        if (! mysqli_query ( $GLOBALS['conn'] , trim ( $sql ) )) {
    ...
    private function lock($tablename, $op = "WRITE") {
        if (mysqli_query ( $GLOBALS['conn'] , "lock tables " . $tablename . " " . $op ))
            return true;
        else
            return false;
    }
    ...
    private function unlock() {
        if (mysqli_query ( $GLOBALS['conn'] , "unlock tables" ))
            return true;
        else
            return false;
    }
    ...
    function __destruct() {
        if($this->db){
            mysqli_query ( $GLOBALS['conn'] , "unlock tables", $this->db );
            mysqli_close ( $this->db );
        }
    }
}
